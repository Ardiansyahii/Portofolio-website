      ...
      <div className={styles.marqueeInner} style={{ '--speed': `${speed}s` }}>
        {[...items, ...items].map((skill, i) => (
          <span key={i} className={styles.marqueePill}>
            <span className={styles.marqueeDot} style={{ background: skill.color }} />
            {skill.name}
          </span>
        ))}
      </div>
      ...
      <div className={styles.bar}>
        <div
          ...
          style={{
            ...
            transitionDelay: `${0.3 + (index % 8) * 0.04}s`,
          }}
        />
      </div>
    </div>
...
export default function Skills() {
  ...
  const filtered = active === 'all'
    ? SKILLS
    : SKILLS.filter(s => s.cats.includes(active))

  return (
    <section id="skills" className={styles.skills}>
      <div className={styles.gradientTop} aria-hidden="true" />

      {/* Marquee — decorative top */}
      ...
      <div className="container" style={{ marginTop: '4rem' }}>
        ...
        {/* Filter pills */}
        <div className={`${styles.filter} reveal ${hView ? 'in' : ''}`} style={{ transitionDelay: '0.1s' }} role="group" aria-label="Filter by category">
          {SKILL_CATEGORIES.map(({ key, label }) => (
            ...
            >
              ...
              <span className={styles.filterCount}>
                {key === 'all' ? SKILLS.length : SKILLS.filter(s => s.cats.includes(key)).length}
              </span>
            </button>
          ))}
        </div>
